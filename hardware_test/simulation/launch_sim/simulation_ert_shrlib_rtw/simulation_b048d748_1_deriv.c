/* Simscape target specific file.
 * This file is generated for the Simscape network associated with the solver block 'simulation/Plant/ World Setup/Solver Configuration'.
 */

#include <math.h>
#include <string.h>
#include "pm_std.h"
#include "sm_std.h"
#include "ne_std.h"
#include "ne_dae.h"
#include "sm_ssci_run_time_errors.h"
#include "sm_RuntimeDerivedValuesBundle.h"
#include "simulation_b048d748_1_geometries.h"

PmfMessageId simulation_b048d748_1_compDerivs(const RuntimeDerivedValuesBundle
  *rtdv, const int *eqnEnableFlags, const double *state, const int *modeVector,
  const double *input, const double *inputDot, const double *inputDdot, const
  double *discreteState, double *deriv, double *errorResult,
  NeuDiagnosticManager *neDiagMgr)
{
  const double *rtdvd = rtdv->mDoubles.mValues;
  const int *rtdvi = rtdv->mInts.mValues;
  boolean_T bb[2];
  int ii[6];
  double xx[551];
  (void) rtdvd;
  (void) rtdvi;
  (void) eqnEnableFlags;
  (void) discreteState;
  (void) neDiagMgr;
  xx[0] = state[3];
  xx[1] = state[4];
  xx[2] = state[5];
  xx[3] = state[6];
  xx[4] = state[10];
  xx[5] = state[11];
  xx[6] = state[12];
  pm_math_Quaternion_compDeriv_ra(xx + 0, xx + 4, xx + 7);
  xx[0] = 1.0;
  xx[1] = state[5] * state[5];
  xx[2] = state[6] * state[6];
  xx[3] = 2.0;
  xx[11] = (xx[1] + xx[2]) * xx[3];
  xx[12] = state[4] * state[5];
  xx[13] = state[3] * state[6];
  xx[14] = xx[3] * (xx[12] - xx[13]);
  xx[15] = state[3] * state[5];
  xx[16] = state[4] * state[6];
  xx[17] = (xx[15] + xx[16]) * xx[3];
  xx[18] = xx[0] - xx[11];
  xx[19] = xx[14];
  xx[20] = xx[17];
  xx[21] = 1.1;
  xx[22] = 0.5;
  xx[23] = xx[22] * input[0];
  xx[24] = cos(xx[23]);
  xx[25] = xx[22] * input[1];
  xx[22] = cos(xx[25]);
  xx[26] = xx[24] * xx[22];
  xx[27] = xx[26] * xx[26];
  xx[28] = sin(xx[23]);
  xx[23] = xx[22] * xx[28];
  xx[29] = xx[23] * xx[23];
  xx[30] = (xx[27] + xx[29]) * xx[3] - xx[0];
  xx[31] = sin(xx[25]);
  xx[32] = xx[24] * xx[31];
  xx[24] = xx[32] * xx[23];
  xx[33] = xx[28] * xx[31];
  xx[28] = xx[26] * xx[33];
  xx[34] = xx[24] - xx[28];
  xx[35] = xx[3] * xx[34];
  xx[36] = xx[23] * xx[33];
  xx[37] = xx[26] * xx[32];
  xx[38] = (xx[36] + xx[37]) * xx[3];
  xx[39] = (xx[24] + xx[28]) * xx[3];
  xx[40] = xx[32] * xx[32];
  xx[41] = (xx[27] + xx[40]) * xx[3] - xx[0];
  xx[42] = xx[32] * xx[33];
  xx[43] = xx[26] * xx[23];
  xx[44] = xx[3] * (xx[42] - xx[43]);
  xx[45] = xx[3] * (xx[36] - xx[37]);
  xx[46] = (xx[42] + xx[43]) * xx[3];
  xx[47] = xx[33] * xx[33];
  xx[48] = (xx[27] + xx[47]) * xx[3] - xx[0];
  xx[49] = xx[30];
  xx[50] = xx[35];
  xx[51] = xx[38];
  xx[52] = xx[39];
  xx[53] = xx[41];
  xx[54] = xx[44];
  xx[55] = xx[45];
  xx[56] = xx[46];
  xx[57] = xx[48];
  xx[27] = 0.1;
  xx[58] = xx[27] * xx[30];
  xx[59] = xx[27] * xx[39];
  xx[60] = xx[27] * xx[45];
  xx[61] = xx[27] * xx[35];
  xx[62] = xx[27] * xx[41];
  xx[63] = xx[27] * xx[46];
  xx[64] = xx[27] * xx[38];
  xx[65] = xx[27] * xx[44];
  xx[66] = xx[27] * xx[48];
  pm_math_Matrix3x3_compose_ra(xx + 49, xx + 58, xx + 67);
  xx[58] = xx[21] + xx[67];
  xx[59] = xx[21] + xx[71];
  xx[76] = xx[58];
  xx[77] = xx[68];
  xx[78] = xx[69];
  xx[79] = xx[70];
  xx[80] = xx[59];
  xx[81] = xx[72];
  xx[82] = xx[73];
  xx[83] = xx[74];
  xx[84] = xx[21] + xx[75];
  pm_math_Matrix3x3_xform_ra(xx + 76, xx + 18, xx + 60);
  xx[21] = (xx[13] + xx[12]) * xx[3];
  xx[63] = state[4] * state[4];
  xx[64] = (xx[2] + xx[63]) * xx[3];
  xx[65] = state[5] * state[6];
  xx[66] = state[3] * state[4];
  xx[85] = xx[3] * (xx[65] - xx[66]);
  xx[86] = xx[21];
  xx[87] = xx[0] - xx[64];
  xx[88] = xx[85];
  pm_math_Matrix3x3_xform_ra(xx + 76, xx + 86, xx + 89);
  xx[92] = pm_math_Vector3_dot_ra(xx + 18, xx + 89);
  xx[93] = xx[3] * (xx[16] - xx[15]);
  xx[94] = (xx[66] + xx[65]) * xx[3];
  xx[95] = (xx[63] + xx[1]) * xx[3];
  xx[96] = xx[93];
  xx[97] = xx[94];
  xx[98] = xx[0] - xx[95];
  pm_math_Matrix3x3_xform_ra(xx + 76, xx + 96, xx + 99);
  xx[102] = pm_math_Vector3_dot_ra(xx + 18, xx + 99);
  xx[103] = 0.04499999999999999;
  xx[104] = xx[103] * xx[32];
  xx[105] = xx[104] * xx[26];
  xx[106] = xx[103] * xx[23];
  xx[107] = (xx[105] + xx[106] * xx[33]) * xx[3];
  xx[108] = xx[3] * (xx[106] * xx[26] - xx[104] * xx[33]);
  xx[109] = xx[104] * xx[32];
  xx[110] = (xx[106] * xx[23] + xx[109]) * xx[3] - xx[103] - xx[0];
  xx[111] = - xx[107];
  xx[112] = xx[108];
  xx[113] = xx[110];
  pm_math_Matrix3x3_postCross_ra(xx + 67, xx + 111, xx + 114);
  xx[67] = - (xx[114] + xx[68]);
  xx[68] = xx[117] + xx[59];
  xx[59] = - xx[68];
  xx[69] = - (xx[120] + xx[74]);
  xx[123] = xx[67];
  xx[124] = xx[59];
  xx[125] = xx[69];
  xx[71] = pm_math_Vector3_dot_ra(xx + 18, xx + 123);
  xx[72] = xx[58] - xx[115];
  xx[58] = xx[70] - xx[118];
  xx[70] = xx[73] - xx[121];
  xx[73] = xx[72];
  xx[74] = xx[58];
  xx[75] = xx[70];
  xx[106] = pm_math_Vector3_dot_ra(xx + 18, xx + 73);
  xx[126] = - xx[116];
  xx[127] = - xx[119];
  xx[128] = - xx[122];
  xx[129] = pm_math_Vector3_dot_ra(xx + 18, xx + 126);
  xx[130] = pm_math_Vector3_dot_ra(xx + 86, xx + 99);
  xx[131] = pm_math_Vector3_dot_ra(xx + 86, xx + 123);
  xx[132] = pm_math_Vector3_dot_ra(xx + 86, xx + 73);
  xx[133] = pm_math_Vector3_dot_ra(xx + 86, xx + 126);
  xx[134] = pm_math_Vector3_dot_ra(xx + 96, xx + 123);
  xx[123] = pm_math_Vector3_dot_ra(xx + 96, xx + 73);
  xx[73] = pm_math_Vector3_dot_ra(xx + 96, xx + 126);
  xx[74] = 1.181250000000001e-4;
  xx[75] = 1.0125e-4;
  xx[135] = xx[74] * xx[30];
  xx[136] = xx[74] * xx[39];
  xx[137] = xx[74] * xx[45];
  xx[138] = xx[74] * xx[35];
  xx[139] = xx[74] * xx[41];
  xx[140] = xx[74] * xx[46];
  xx[141] = xx[75] * xx[38];
  xx[142] = xx[75] * xx[44];
  xx[143] = xx[75] * xx[48];
  pm_math_Matrix3x3_compose_ra(xx + 49, xx + 135, xx + 144);
  pm_math_Matrix3x3_preCross_ra(xx + 114, xx + 111, xx + 135);
  xx[30] = xx[0] + xx[144] - xx[135] + xx[117];
  xx[38] = xx[145] - xx[136] - xx[114];
  xx[39] = xx[38] - xx[58];
  xx[41] = xx[146] - xx[137];
  xx[46] = xx[41] + xx[119];
  xx[48] = xx[0] + xx[148] - xx[139] - xx[115];
  xx[122] = xx[149] - xx[140];
  xx[124] = xx[122] - xx[116];
  xx[125] = xx[0] + xx[152] - xx[143];
  xx[153] = pm_math_Vector3_dot_ra(xx + 18, xx + 60);
  xx[154] = xx[92];
  xx[155] = xx[102];
  xx[156] = xx[71];
  xx[157] = xx[106];
  xx[158] = xx[129];
  xx[159] = xx[92];
  xx[160] = pm_math_Vector3_dot_ra(xx + 86, xx + 89);
  xx[161] = xx[130];
  xx[162] = xx[131];
  xx[163] = xx[132];
  xx[164] = xx[133];
  xx[165] = xx[102];
  xx[166] = xx[130];
  xx[167] = pm_math_Vector3_dot_ra(xx + 96, xx + 99);
  xx[168] = xx[134];
  xx[169] = xx[123];
  xx[170] = xx[73];
  xx[171] = xx[71];
  xx[172] = xx[131];
  xx[173] = xx[134];
  xx[174] = xx[30] + xx[68];
  xx[175] = xx[39];
  xx[176] = xx[46];
  xx[177] = xx[106];
  xx[178] = xx[132];
  xx[179] = xx[123];
  xx[180] = xx[39];
  xx[181] = xx[48] + xx[72];
  xx[182] = xx[124];
  xx[183] = xx[129];
  xx[184] = xx[133];
  xx[185] = xx[73];
  xx[186] = xx[46];
  xx[187] = xx[124];
  xx[188] = xx[125];
  ii[0] = factorSymmetricPosDef(xx + 153, 6, xx + 129);
  if (ii[0] != 0) {
    return sm_ssci_recordRunTimeError(
      "physmod:sm:core:compiler:mechanism:mechanism:degenerateMassFoll",
      "'simulation/Plant/ Rocket Body/Free Movement' has a degenerate mass distribution on its follower side.",
      neDiagMgr);
  }

  xx[39] = 0.0;
  bb[0] = modeVector[0] == -1;
  xx[46] = - state[3];
  xx[68] = - state[4];
  xx[71] = - state[5];
  xx[73] = - state[6];
  xx[129] = xx[46];
  xx[130] = xx[68];
  xx[131] = xx[71];
  xx[132] = xx[73];
  xx[92] = - xx[23];
  xx[102] = - xx[32];
  xx[106] = - xx[33];
  xx[189] = - xx[26];
  xx[190] = xx[92];
  xx[191] = xx[102];
  xx[192] = xx[106];
  pm_math_Quaternion_compose_ra(xx + 129, xx + 189, xx + 193);
  xx[123] = xx[0] - (xx[40] + xx[47]) * xx[3];
  xx[124] = xx[195] * xx[123];
  xx[133] = (xx[37] + xx[36]) * xx[3];
  xx[36] = xx[196] * xx[133];
  xx[37] = xx[194] * xx[123];
  xx[134] = xx[36] + xx[37];
  xx[197] = xx[195] * xx[133];
  xx[198] = xx[124];
  xx[199] = - xx[134];
  xx[200] = xx[197];
  pm_math_Vector3_cross_ra(xx + 194, xx + 198, xx + 201);
  xx[198] = (xx[193] * xx[124] + xx[201]) * xx[3] - xx[133];
  xx[199] = xx[3] * (xx[202] - xx[134] * xx[193]);
  xx[200] = xx[123] + (xx[193] * xx[197] + xx[203]) * xx[3];
  xx[124] = state[16] * state[16];
  xx[134] = state[17] * state[17];
  xx[197] = (xx[124] + xx[134]) * xx[3] - xx[0];
  xx[201] = state[17] * state[18];
  xx[202] = state[16] * state[19];
  xx[203] = (xx[201] + xx[202]) * xx[3];
  xx[204] = state[17] * state[19];
  xx[205] = state[16] * state[18];
  xx[206] = xx[3] * (xx[204] - xx[205]);
  xx[207] = xx[197];
  xx[208] = xx[203];
  xx[209] = xx[206];
  xx[210] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 198, xx + 207);
  xx[198] = (xx[28] + xx[24]) * xx[3];
  xx[24] = xx[195] * xx[198];
  xx[28] = xx[196] * xx[44];
  xx[199] = xx[194] * xx[198];
  xx[200] = xx[28] + xx[199];
  xx[211] = xx[195] * xx[44];
  xx[212] = xx[24];
  xx[213] = - xx[200];
  xx[214] = xx[211];
  pm_math_Vector3_cross_ra(xx + 194, xx + 212, xx + 215);
  xx[212] = (xx[193] * xx[24] + xx[215]) * xx[3] - xx[44];
  xx[213] = xx[3] * (xx[216] - xx[200] * xx[193]);
  xx[214] = xx[198] + (xx[193] * xx[211] + xx[217]) * xx[3];
  xx[24] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 212, xx + 207);
  xx[200] = xx[195] * xx[45];
  xx[211] = xx[0] - (xx[29] + xx[40]) * xx[3];
  xx[40] = xx[196] * xx[211];
  xx[212] = xx[194] * xx[45];
  xx[213] = xx[40] + xx[212];
  xx[214] = xx[195] * xx[211];
  xx[215] = xx[200];
  xx[216] = - xx[213];
  xx[217] = xx[214];
  pm_math_Vector3_cross_ra(xx + 194, xx + 215, xx + 218);
  xx[215] = (xx[193] * xx[200] + xx[218]) * xx[3] - xx[211];
  xx[216] = xx[3] * (xx[219] - xx[213] * xx[193]);
  xx[217] = xx[45] + (xx[193] * xx[214] + xx[220]) * xx[3];
  xx[200] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 215, xx + 207);
  xx[213] = xx[39];
  xx[214] = xx[39];
  xx[215] = xx[39];
  xx[216] = xx[210];
  xx[217] = xx[24];
  xx[218] = xx[200];
  solveSymmetricPosDef(xx + 153, xx + 213, 6, 1, xx + 219, xx + 225);
  xx[213] = state[18] * state[18];
  xx[214] = state[19] * state[19];
  xx[215] = (xx[213] + xx[214]) * xx[3];
  xx[216] = xx[0] - xx[215];
  xx[217] = xx[3] * (xx[201] - xx[202]);
  xx[218] = (xx[205] + xx[204]) * xx[3];
  xx[219] = xx[27] * xx[216];
  xx[220] = xx[27] * xx[217];
  xx[221] = xx[27] * xx[218];
  xx[225] = (xx[202] + xx[201]) * xx[3];
  xx[226] = xx[27] * xx[225];
  xx[227] = (xx[214] + xx[134]) * xx[3];
  xx[228] = xx[0] - xx[227];
  xx[229] = xx[27] * xx[228];
  xx[230] = state[18] * state[19];
  xx[231] = state[16] * state[17];
  xx[232] = xx[3] * (xx[230] - xx[231]);
  xx[233] = xx[27] * xx[232];
  xx[234] = xx[226];
  xx[235] = xx[229];
  xx[236] = xx[233];
  xx[237] = pm_math_Vector3_dot_ra(xx + 216, xx + 234);
  xx[238] = xx[27] * xx[206];
  xx[239] = (xx[231] + xx[230]) * xx[3];
  xx[240] = xx[27] * xx[239];
  xx[241] = (xx[134] + xx[213]) * xx[3];
  xx[134] = xx[0] - xx[241];
  xx[242] = xx[27] * xx[134];
  xx[243] = xx[238];
  xx[244] = xx[240];
  xx[245] = xx[242];
  xx[246] = pm_math_Vector3_dot_ra(xx + 216, xx + 243);
  xx[247] = xx[225];
  xx[248] = xx[228];
  xx[249] = xx[232];
  xx[250] = pm_math_Vector3_dot_ra(xx + 247, xx + 243);
  xx[251] = xx[206];
  xx[252] = xx[239];
  xx[253] = xx[134];
  xx[254] = 6.750000000000001e-5;
  xx[255] = pm_math_Vector3_dot_ra(xx + 216, xx + 219);
  xx[256] = xx[237];
  xx[257] = xx[246];
  xx[258] = xx[39];
  xx[259] = xx[39];
  xx[260] = xx[39];
  xx[261] = xx[237];
  xx[262] = pm_math_Vector3_dot_ra(xx + 247, xx + 234);
  xx[263] = xx[250];
  xx[264] = xx[39];
  xx[265] = xx[39];
  xx[266] = xx[39];
  xx[267] = xx[246];
  xx[268] = xx[250];
  xx[269] = pm_math_Vector3_dot_ra(xx + 251, xx + 243);
  xx[270] = xx[39];
  xx[271] = xx[39];
  xx[272] = xx[39];
  xx[273] = xx[39];
  xx[274] = xx[39];
  xx[275] = xx[39];
  xx[276] = xx[254];
  xx[277] = xx[39];
  xx[278] = xx[39];
  xx[279] = xx[39];
  xx[280] = xx[39];
  xx[281] = xx[39];
  xx[282] = xx[39];
  xx[283] = xx[254];
  xx[284] = xx[39];
  xx[285] = xx[39];
  xx[286] = xx[39];
  xx[287] = xx[39];
  xx[288] = xx[39];
  xx[289] = xx[39];
  xx[290] = xx[75];
  ii[0] = factorSymmetricPosDef(xx + 255, 6, xx + 291);
  if (ii[0] != 0) {
    return sm_ssci_recordRunTimeError(
      "physmod:sm:core:compiler:mechanism:mechanism:degenerateMassImplicit6Dof",
      "An implicit 6-DOF joint is attached to a degenerate mass distribution.",
      neDiagMgr);
  }

  xx[234] = - xx[218];
  xx[235] = xx[3] * (xx[231] - xx[230]);
  xx[236] = xx[241] - xx[0];
  xx[237] = xx[194] * xx[195];
  xx[241] = xx[193] * xx[196];
  xx[243] = xx[3] * (xx[237] - xx[241]);
  xx[244] = xx[193] * xx[193];
  xx[245] = xx[195] * xx[195];
  xx[246] = (xx[244] + xx[245]) * xx[3] - xx[0];
  xx[250] = xx[195] * xx[196];
  xx[291] = xx[193] * xx[194];
  xx[292] = (xx[250] + xx[291]) * xx[3];
  xx[293] = xx[243];
  xx[294] = xx[246];
  xx[295] = xx[292];
  xx[296] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 234, xx + 293);
  xx[234] = xx[217];
  xx[235] = xx[228];
  xx[236] = xx[239];
  xx[228] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 234, xx + 293);
  xx[297] = xx[39];
  xx[298] = xx[39];
  xx[299] = xx[39];
  xx[300] = xx[39];
  xx[301] = xx[296];
  xx[302] = xx[228];
  solveSymmetricPosDef(xx + 255, xx + 297, 6, 1, xx + 303, xx + 309);
  xx[234] = xx[196] * xx[123];
  xx[235] = xx[196] * xx[35];
  xx[236] = xx[195] * xx[35];
  xx[297] = xx[37] + xx[236];
  xx[298] = xx[234];
  xx[299] = xx[235];
  xx[300] = - xx[297];
  pm_math_Vector3_cross_ra(xx + 194, xx + 298, xx + 301);
  xx[298] = xx[35] + (xx[193] * xx[234] + xx[301]) * xx[3];
  xx[299] = (xx[193] * xx[235] + xx[302]) * xx[3] - xx[123];
  xx[300] = xx[3] * (xx[303] - xx[297] * xx[193]);
  xx[37] = (xx[124] + xx[213]) * xx[3] - xx[0];
  xx[213] = (xx[230] + xx[231]) * xx[3];
  xx[301] = xx[217];
  xx[302] = xx[37];
  xx[303] = xx[213];
  xx[230] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 298, xx + 301);
  xx[231] = xx[0] - (xx[47] + xx[29]) * xx[3];
  xx[29] = xx[196] * xx[198];
  xx[47] = xx[196] * xx[231];
  xx[234] = xx[195] * xx[231];
  xx[235] = xx[199] + xx[234];
  xx[297] = xx[29];
  xx[298] = xx[47];
  xx[299] = - xx[235];
  pm_math_Vector3_cross_ra(xx + 194, xx + 297, xx + 304);
  xx[297] = xx[231] + (xx[193] * xx[29] + xx[304]) * xx[3];
  xx[298] = (xx[193] * xx[47] + xx[305]) * xx[3] - xx[198];
  xx[299] = xx[3] * (xx[306] - xx[235] * xx[193]);
  xx[29] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 297, xx + 301);
  xx[47] = (xx[43] + xx[42]) * xx[3];
  xx[42] = xx[196] * xx[45];
  xx[43] = xx[196] * xx[47];
  xx[199] = xx[195] * xx[47];
  xx[235] = xx[212] + xx[199];
  xx[297] = xx[42];
  xx[298] = xx[43];
  xx[299] = - xx[235];
  pm_math_Vector3_cross_ra(xx + 194, xx + 297, xx + 304);
  xx[297] = xx[47] + (xx[193] * xx[42] + xx[304]) * xx[3];
  xx[298] = (xx[193] * xx[43] + xx[305]) * xx[3] - xx[45];
  xx[299] = xx[3] * (xx[306] - xx[235] * xx[193]);
  xx[42] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 297, xx + 301);
  xx[309] = xx[39];
  xx[310] = xx[39];
  xx[311] = xx[39];
  xx[312] = xx[230];
  xx[313] = xx[29];
  xx[314] = xx[42];
  solveSymmetricPosDef(xx + 153, xx + 309, 6, 1, xx + 315, xx + 321);
  xx[297] = xx[218];
  xx[298] = xx[232];
  xx[299] = xx[134];
  xx[43] = xx[194] * xx[196];
  xx[134] = xx[193] * xx[195];
  xx[212] = (xx[43] + xx[134]) * xx[3];
  xx[235] = xx[3] * (xx[250] - xx[291]);
  xx[300] = xx[196] * xx[196];
  xx[304] = (xx[244] + xx[300]) * xx[3] - xx[0];
  xx[309] = xx[212];
  xx[310] = xx[235];
  xx[311] = xx[304];
  xx[305] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 297, xx + 309);
  xx[297] = xx[215] - xx[0];
  xx[298] = - xx[225];
  xx[299] = xx[3] * (xx[205] - xx[204]);
  xx[215] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 297, xx + 309);
  xx[312] = xx[39];
  xx[313] = xx[39];
  xx[314] = xx[39];
  xx[315] = xx[305];
  xx[316] = xx[39];
  xx[317] = xx[215];
  solveSymmetricPosDef(xx + 255, xx + 312, 6, 1, xx + 321, xx + 327);
  xx[297] = xx[318] * xx[210] + xx[319] * xx[24] + xx[320] * xx[200] + xx[325] *
    xx[296] + xx[326] * xx[228];
  xx[298] = xx[236] + xx[36];
  xx[36] = xx[194] * xx[35];
  xx[236] = xx[194] * xx[133];
  xx[312] = - xx[298];
  xx[313] = xx[36];
  xx[314] = xx[236];
  pm_math_Vector3_cross_ra(xx + 194, xx + 312, xx + 315);
  xx[312] = xx[3] * (xx[315] - xx[298] * xx[193]);
  xx[313] = xx[133] + (xx[193] * xx[36] + xx[316]) * xx[3];
  xx[314] = (xx[193] * xx[236] + xx[317]) * xx[3] - xx[35];
  xx[36] = (xx[204] + xx[205]) * xx[3];
  xx[204] = (xx[124] + xx[214]) * xx[3] - xx[0];
  xx[315] = xx[36];
  xx[316] = xx[232];
  xx[317] = xx[204];
  xx[124] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 312, xx + 315);
  xx[205] = xx[234] + xx[28];
  xx[28] = xx[194] * xx[231];
  xx[214] = xx[194] * xx[44];
  xx[312] = - xx[205];
  xx[313] = xx[28];
  xx[314] = xx[214];
  pm_math_Vector3_cross_ra(xx + 194, xx + 312, xx + 327);
  xx[312] = xx[3] * (xx[327] - xx[205] * xx[193]);
  xx[313] = xx[44] + (xx[193] * xx[28] + xx[328]) * xx[3];
  xx[314] = (xx[193] * xx[214] + xx[329]) * xx[3] - xx[231];
  xx[28] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 312, xx + 315);
  xx[44] = xx[199] + xx[40];
  xx[40] = xx[194] * xx[47];
  xx[199] = xx[194] * xx[211];
  xx[312] = - xx[44];
  xx[313] = xx[40];
  xx[314] = xx[199];
  pm_math_Vector3_cross_ra(xx + 194, xx + 312, xx + 327);
  xx[312] = xx[3] * (xx[327] - xx[44] * xx[193]);
  xx[313] = xx[211] + (xx[193] * xx[40] + xx[328]) * xx[3];
  xx[314] = (xx[193] * xx[199] + xx[329]) * xx[3] - xx[47];
  xx[40] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 312, xx + 315);
  xx[327] = xx[39];
  xx[328] = xx[39];
  xx[329] = xx[39];
  xx[330] = xx[124];
  xx[331] = xx[28];
  xx[332] = xx[40];
  solveSymmetricPosDef(xx + 153, xx + 327, 6, 1, xx + 333, xx + 339);
  xx[312] = xx[3] * (xx[202] - xx[201]);
  xx[313] = xx[227] - xx[0];
  xx[314] = - xx[239];
  xx[44] = xx[194] * xx[194];
  xx[199] = (xx[244] + xx[44]) * xx[3] - xx[0];
  xx[201] = (xx[237] + xx[241]) * xx[3];
  xx[202] = xx[3] * (xx[43] - xx[134]);
  xx[327] = xx[199];
  xx[328] = xx[201];
  xx[329] = xx[202];
  xx[205] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 312, xx + 327);
  xx[312] = xx[216];
  xx[313] = xx[225];
  xx[314] = xx[206];
  xx[211] = bb[0] ? xx[39] : pm_math_Vector3_dot_ra(xx + 312, xx + 327);
  xx[330] = xx[39];
  xx[331] = xx[39];
  xx[332] = xx[39];
  xx[333] = xx[205];
  xx[334] = xx[211];
  xx[335] = xx[39];
  solveSymmetricPosDef(xx + 255, xx + 330, 6, 1, xx + 339, xx + 345);
  xx[214] = xx[336] * xx[210] + xx[337] * xx[24] + xx[338] * xx[200] + xx[343] *
    xx[296] + xx[344] * xx[228];
  xx[225] = - xx[0];
  xx[227] = bb[0] ? xx[39] : xx[225];
  xx[234] = 0.04500000000000001;
  xx[236] = xx[234] * xx[123];
  xx[239] = xx[196] * xx[236];
  xx[244] = xx[234] * xx[35];
  xx[298] = xx[196] * xx[244];
  xx[299] = xx[194] * xx[236] + xx[195] * xx[244];
  xx[312] = - xx[239];
  xx[313] = - xx[298];
  xx[314] = xx[299];
  pm_math_Vector3_cross_ra(xx + 194, xx + 312, xx + 330);
  xx[306] = (xx[330] - xx[193] * xx[239]) * xx[3] - xx[244];
  xx[239] = xx[108] * state[5];
  xx[244] = xx[110] * state[6];
  xx[312] = xx[239] + xx[244];
  xx[333] = xx[68];
  xx[334] = xx[71];
  xx[335] = xx[73];
  xx[313] = xx[108] * state[4];
  xx[314] = xx[110] * state[4];
  xx[345] = - xx[312];
  xx[346] = xx[313];
  xx[347] = xx[314];
  pm_math_Vector3_cross_ra(xx + 333, xx + 345, xx + 348);
  xx[345] = bb[0] ? xx[39] : - (xx[306] + (xx[312] * state[3] + xx[348]) * xx[3]
    + xx[3] * (xx[13] - xx[12]));
  xx[312] = xx[234] * xx[198];
  xx[198] = xx[196] * xx[312];
  xx[346] = xx[234] * xx[231];
  xx[231] = xx[196] * xx[346];
  xx[347] = xx[194] * xx[312] + xx[195] * xx[346];
  xx[351] = - xx[198];
  xx[352] = - xx[231];
  xx[353] = xx[347];
  pm_math_Vector3_cross_ra(xx + 194, xx + 351, xx + 354);
  xx[351] = xx[107] * state[5];
  xx[352] = xx[107] * state[4];
  xx[353] = xx[244] - xx[352];
  xx[244] = xx[110] * state[5];
  xx[357] = - xx[351];
  xx[358] = - xx[353];
  xx[359] = xx[244];
  pm_math_Vector3_cross_ra(xx + 333, xx + 357, xx + 360);
  xx[357] = bb[0] ? xx[39] : - ((xx[354] - xx[193] * xx[198]) * xx[3] - xx[346]
    + xx[110] + xx[3] * (xx[360] + xx[351] * state[3]) - xx[11] + xx[0]);
  xx[11] = xx[234] * xx[45];
  xx[45] = xx[196] * xx[11];
  xx[198] = xx[234] * xx[47];
  xx[47] = xx[196] * xx[198];
  xx[346] = xx[194] * xx[11] + xx[195] * xx[198];
  xx[363] = - xx[45];
  xx[364] = - xx[47];
  xx[365] = xx[346];
  pm_math_Vector3_cross_ra(xx + 194, xx + 363, xx + 366);
  xx[351] = xx[107] * state[6];
  xx[358] = xx[108] * state[6];
  xx[359] = xx[239] - xx[352];
  xx[363] = - xx[351];
  xx[364] = xx[358];
  xx[365] = - xx[359];
  pm_math_Vector3_cross_ra(xx + 333, xx + 363, xx + 369);
  xx[239] = bb[0] ? xx[39] : - ((xx[366] - xx[193] * xx[45]) * xx[3] - xx[198] +
    xx[3] * (xx[369] + xx[351] * state[3]) - xx[108]);
  xx[372] = xx[227];
  xx[373] = xx[39];
  xx[374] = xx[39];
  xx[375] = xx[345];
  xx[376] = xx[357];
  xx[377] = xx[239];
  solveSymmetricPosDef(xx + 153, xx + 372, 6, 1, xx + 378, xx + 384);
  xx[45] = bb[0] ? xx[39] : xx[0];
  xx[198] = 0.0225;
  xx[333] = xx[198] * state[19];
  xx[334] = xx[333] * state[16];
  xx[335] = xx[198] * state[17];
  xx[351] = bb[0] ? xx[39] : xx[3] * (xx[334] - xx[335] * state[18]);
  xx[352] = xx[198] * state[18];
  xx[363] = xx[333] * state[19];
  xx[364] = bb[0] ? xx[39] : xx[198] - (xx[352] * state[18] + xx[363]) * xx[3];
  xx[372] = xx[45];
  xx[373] = xx[39];
  xx[374] = xx[39];
  xx[375] = xx[351];
  xx[376] = xx[364];
  xx[377] = xx[39];
  solveSymmetricPosDef(xx + 255, xx + 372, 6, 1, xx + 384, xx + 390);
  xx[365] = xx[381] * xx[210] + xx[382] * xx[24] + xx[383] * xx[200] + xx[388] *
    xx[296] + xx[389] * xx[228];
  xx[372] = (xx[331] - xx[193] * xx[298]) * xx[3];
  xx[298] = bb[0] ? xx[39] : - (xx[372] + xx[3] * (xx[349] - xx[313] * state[3])
    + xx[64] - xx[110] + xx[236] - xx[0]);
  xx[64] = bb[0] ? xx[39] : - ((xx[355] - xx[193] * xx[231]) * xx[3] + xx[312] +
    (xx[353] * state[3] + xx[361]) * xx[3] + xx[21]);
  xx[21] = bb[0] ? xx[39] : - ((xx[367] - xx[193] * xx[47]) * xx[3] + xx[11] +
    xx[3] * (xx[370] - xx[358] * state[3]) - xx[107]);
  xx[390] = xx[39];
  xx[391] = xx[227];
  xx[392] = xx[39];
  xx[393] = xx[298];
  xx[394] = xx[64];
  xx[395] = xx[21];
  solveSymmetricPosDef(xx + 153, xx + 390, 6, 1, xx + 396, xx + 402);
  xx[11] = bb[0] ? xx[39] : (xx[363] + xx[335] * state[17]) * xx[3] - xx[198];
  xx[47] = bb[0] ? xx[39] : (xx[334] + xx[352] * state[17]) * xx[3];
  xx[390] = xx[39];
  xx[391] = xx[45];
  xx[392] = xx[39];
  xx[393] = xx[11];
  xx[394] = xx[47];
  xx[395] = xx[39];
  solveSymmetricPosDef(xx + 255, xx + 390, 6, 1, xx + 402, xx + 408);
  xx[110] = xx[399] * xx[210] + xx[400] * xx[24] + xx[401] * xx[200] + xx[406] *
    xx[296] + xx[407] * xx[228];
  xx[231] = xx[3] * (xx[332] + xx[299] * xx[193]);
  xx[299] = bb[0] ? xx[39] : - (xx[231] + xx[108] + xx[3] * (xx[350] - xx[314] *
    state[3]) - xx[94]);
  xx[94] = bb[0] ? xx[39] : - (xx[3] * (xx[356] + xx[347] * xx[193]) + xx[3] *
    (xx[362] - xx[244] * state[3]) + xx[107] + xx[93]);
  xx[107] = bb[0] ? xx[39] : - (xx[3] * (xx[368] + xx[346] * xx[193]) + (xx[359]
    * state[3] + xx[371]) * xx[3]);
  xx[358] = xx[39];
  xx[359] = xx[39];
  xx[360] = xx[227];
  xx[361] = xx[299];
  xx[362] = xx[94];
  xx[363] = xx[107];
  solveSymmetricPosDef(xx + 153, xx + 358, 6, 1, xx + 366, xx + 390);
  xx[108] = bb[0] ? xx[39] : - ((xx[335] * state[16] + xx[333] * state[18]) *
    xx[3]);
  xx[244] = bb[0] ? xx[39] : xx[3] * (xx[333] * state[17] - xx[352] * state[16]);
  xx[330] = xx[39];
  xx[331] = xx[39];
  xx[332] = xx[45];
  xx[333] = xx[108];
  xx[334] = xx[244];
  xx[335] = xx[39];
  solveSymmetricPosDef(xx + 255, xx + 330, 6, 1, xx + 358, xx + 390);
  xx[312] = xx[369] * xx[210] + xx[370] * xx[24] + xx[371] * xx[200] + xx[362] *
    xx[296] + xx[363] * xx[228];
  xx[313] = xx[336] * xx[230] + xx[337] * xx[29] + xx[338] * xx[42] + xx[342] *
    xx[305] + xx[344] * xx[215];
  xx[314] = xx[381] * xx[230] + xx[382] * xx[29] + xx[383] * xx[42] + xx[387] *
    xx[305] + xx[389] * xx[215];
  xx[321] = xx[399] * xx[230] + xx[400] * xx[29] + xx[401] * xx[42] + xx[405] *
    xx[305] + xx[407] * xx[215];
  xx[322] = xx[369] * xx[230] + xx[370] * xx[29] + xx[371] * xx[42] + xx[361] *
    xx[305] + xx[363] * xx[215];
  xx[323] = xx[381] * xx[124] + xx[382] * xx[28] + xx[383] * xx[40] + xx[387] *
    xx[205] + xx[388] * xx[211];
  xx[325] = xx[399] * xx[124] + xx[400] * xx[28] + xx[401] * xx[40] + xx[405] *
    xx[205] + xx[406] * xx[211];
  xx[330] = xx[369] * xx[124] + xx[370] * xx[28] + xx[371] * xx[40] + xx[361] *
    xx[205] + xx[362] * xx[211];
  xx[331] = xx[396] * xx[227] + xx[399] * xx[345] + xx[400] * xx[357] + xx[401] *
    xx[239] + xx[402] * xx[45] + xx[405] * xx[351] + xx[406] * xx[364];
  xx[332] = xx[366] * xx[227] + xx[369] * xx[345] + xx[370] * xx[357] + xx[371] *
    xx[239] + xx[358] * xx[45] + xx[361] * xx[351] + xx[362] * xx[364];
  xx[333] = xx[367] * xx[227] + xx[369] * xx[298] + xx[370] * xx[64] + xx[371] *
    xx[21] + xx[359] * xx[45] + xx[361] * xx[11] + xx[362] * xx[47];
  xx[407] = xx[222] * xx[210] + xx[223] * xx[24] + xx[224] * xx[200] + xx[307] *
    xx[296] + xx[308] * xx[228];
  xx[408] = xx[297];
  xx[409] = xx[214];
  xx[410] = xx[365];
  xx[411] = xx[110];
  xx[412] = xx[312];
  xx[413] = xx[297];
  xx[414] = xx[318] * xx[230] + xx[319] * xx[29] + xx[320] * xx[42] + xx[324] *
    xx[305] + xx[326] * xx[215];
  xx[415] = xx[313];
  xx[416] = xx[314];
  xx[417] = xx[321];
  xx[418] = xx[322];
  xx[419] = xx[214];
  xx[420] = xx[313];
  xx[421] = xx[336] * xx[124] + xx[337] * xx[28] + xx[338] * xx[40] + xx[342] *
    xx[205] + xx[343] * xx[211];
  xx[422] = xx[323];
  xx[423] = xx[325];
  xx[424] = xx[330];
  xx[425] = xx[365];
  xx[426] = xx[314];
  xx[427] = xx[323];
  xx[428] = xx[378] * xx[227] + xx[381] * xx[345] + xx[382] * xx[357] + xx[383] *
    xx[239] + xx[384] * xx[45] + xx[387] * xx[351] + xx[388] * xx[364];
  xx[429] = xx[331];
  xx[430] = xx[332];
  xx[431] = xx[110];
  xx[432] = xx[321];
  xx[433] = xx[325];
  xx[434] = xx[331];
  xx[435] = xx[397] * xx[227] + xx[399] * xx[298] + xx[400] * xx[64] + xx[401] *
    xx[21] + xx[403] * xx[45] + xx[405] * xx[11] + xx[406] * xx[47];
  xx[436] = xx[333];
  xx[437] = xx[312];
  xx[438] = xx[322];
  xx[439] = xx[330];
  xx[440] = xx[332];
  xx[441] = xx[333];
  xx[442] = xx[368] * xx[227] + xx[369] * xx[299] + xx[370] * xx[94] + xx[371] *
    xx[107] + xx[360] * xx[45] + xx[361] * xx[108] + xx[362] * xx[244];
  ii[0] = bb[0] ? 0 : 1;
  xx[110] = (xx[16] + xx[15]) * xx[3];
  xx[15] = state[3] * state[3];
  xx[16] = (xx[15] + xx[63]) * xx[3] - xx[0];
  xx[63] = xx[110] * state[10] - xx[16] * state[12];
  xx[214] = xx[16] * state[11] - xx[14] * state[10];
  xx[222] = xx[14] * state[12] - xx[110] * state[11];
  xx[223] = xx[222] * state[11] - xx[63] * state[10];
  xx[224] = (xx[12] + xx[13]) * xx[3];
  xx[12] = xx[85] * state[10] - xx[224] * state[12];
  xx[13] = (xx[15] + xx[1]) * xx[3] - xx[0];
  xx[1] = xx[224] * state[11] - xx[13] * state[10];
  xx[297] = xx[13] * state[12] - xx[85] * state[11];
  xx[307] = xx[297] * state[11] - xx[12] * state[10];
  xx[308] = (xx[15] + xx[2]) * xx[3] - xx[0];
  xx[2] = xx[308] * state[10] - xx[93] * state[12];
  xx[15] = (xx[65] + xx[66]) * xx[3];
  xx[312] = xx[93] * state[11] - xx[15] * state[10];
  xx[313] = xx[15] * state[12] - xx[308] * state[11];
  xx[314] = xx[313] * state[11] - xx[2] * state[10];
  xx[318] = xx[63] * state[12] - xx[214] * state[11];
  xx[319] = xx[214] * state[10] - xx[222] * state[12];
  xx[320] = xx[223];
  xx[321] = xx[12] * state[12] - xx[1] * state[11];
  xx[322] = xx[1] * state[10] - xx[297] * state[12];
  xx[323] = xx[307];
  xx[324] = xx[2] * state[12] - xx[312] * state[11];
  xx[325] = xx[312] * state[10] - xx[313] * state[12];
  xx[326] = xx[314];
  pm_math_Matrix3x3_compose_ra(xx + 318, xx + 49, xx + 330);
  xx[373] = xx[16];
  xx[374] = xx[14];
  xx[375] = xx[110];
  xx[376] = xx[224];
  xx[377] = xx[13];
  xx[378] = xx[85];
  xx[379] = xx[93];
  xx[380] = xx[15];
  xx[381] = xx[308];
  xx[1] = xx[133] * inputDot[0];
  xx[2] = xx[1] * xx[1];
  xx[12] = xx[3] * xx[34] * inputDot[0] + inputDot[1];
  xx[13] = xx[12] * xx[12];
  xx[14] = xx[123] * inputDot[0];
  xx[15] = xx[12] * xx[14];
  xx[339] = xx[14];
  xx[340] = xx[12];
  xx[341] = xx[1];
  xx[342] = xx[123];
  xx[343] = xx[35];
  xx[344] = xx[133];
  pm_math_Vector3_cross_ra(xx + 339, xx + 342, xx + 346);
  xx[16] = xx[348] * inputDot[0];
  xx[34] = xx[1] * xx[14];
  xx[63] = xx[347] * inputDot[0];
  xx[85] = xx[14] * xx[14];
  xx[93] = xx[12] * xx[1];
  xx[110] = xx[346] * inputDot[0];
  xx[382] = - (xx[2] + xx[13]);
  xx[383] = xx[15] + xx[16];
  xx[384] = xx[34] - xx[63];
  xx[385] = xx[15] - xx[16];
  xx[386] = - (xx[2] + xx[85]);
  xx[387] = xx[93] + xx[110];
  xx[388] = xx[34] + xx[63];
  xx[389] = xx[93] - xx[110];
  xx[390] = - (xx[13] + xx[85]);
  pm_math_Matrix3x3_compose_ra(xx + 49, xx + 382, xx + 391);
  pm_math_Matrix3x3_compose_ra(xx + 373, xx + 391, xx + 49);
  xx[373] = xx[199];
  xx[374] = xx[243];
  xx[375] = xx[212];
  xx[376] = xx[201];
  xx[377] = xx[246];
  xx[378] = xx[235];
  xx[379] = xx[202];
  xx[380] = xx[292];
  xx[381] = xx[304];
  pm_math_Quaternion_inverseXform_ra(xx + 189, xx + 4, xx + 342);
  pm_math_Matrix3x3_postCross_ra(xx + 373, xx + 342, xx + 382);
  pm_math_Matrix3x3_postCross_ra(xx + 382, xx + 339, xx + 373);
  xx[339] = xx[331] + xx[50] + xx[3] * xx[374];
  xx[340] = xx[334] + xx[53] + xx[3] * xx[377];
  xx[341] = xx[337] + xx[56] + xx[3] * xx[380];
  xx[2] = xx[36] * state[23] - xx[197] * state[25];
  xx[13] = xx[197] * state[24] - xx[217] * state[23];
  xx[15] = xx[232] * state[23] - xx[203] * state[25];
  xx[16] = xx[203] * state[24] - xx[37] * state[23];
  xx[34] = xx[204] * state[23] - xx[206] * state[25];
  xx[63] = xx[206] * state[24] - xx[213] * state[23];
  xx[346] = xx[2] * state[25] - xx[13] * state[24];
  xx[347] = xx[15] * state[25] - xx[16] * state[24];
  xx[348] = xx[34] * state[25] - xx[63] * state[24];
  xx[85] = - state[17];
  xx[93] = - state[18];
  xx[110] = - state[19];
  xx[352] = xx[85];
  xx[353] = xx[93];
  xx[354] = xx[110];
  xx[133] = state[18] * state[24];
  xx[197] = state[19] * state[25];
  xx[199] = xx[133] + xx[197];
  xx[201] = state[17] * state[24];
  xx[203] = state[17] * state[25];
  xx[358] = xx[199];
  xx[359] = - xx[201];
  xx[360] = - xx[203];
  pm_math_Vector3_cross_ra(xx + 352, xx + 358, xx + 361);
  xx[358] = xx[3] * (xx[361] - xx[199] * state[16]);
  xx[359] = state[25] + (xx[201] * state[16] + xx[362]) * xx[3];
  xx[360] = (xx[203] * state[16] + xx[363]) * xx[3] - state[24];
  xx[199] = xx[342] + xx[14];
  xx[201] = xx[199] * xx[195];
  xx[203] = xx[344] + xx[1];
  xx[1] = xx[203] * xx[196];
  xx[206] = xx[199] * xx[194];
  xx[212] = xx[1] + xx[206];
  xx[214] = xx[203] * xx[195];
  xx[361] = xx[201];
  xx[362] = - xx[212];
  xx[363] = xx[214];
  pm_math_Vector3_cross_ra(xx + 194, xx + 361, xx + 365);
  xx[361] = (xx[193] * xx[201] + xx[365]) * xx[3] - xx[203];
  xx[362] = xx[3] * (xx[366] - xx[212] * xx[193]);
  xx[363] = xx[199] + (xx[193] * xx[214] + xx[367]) * xx[3];
  xx[201] = xx[332] + xx[51] + xx[3] * xx[375];
  xx[212] = xx[335] + xx[54] + xx[3] * xx[378];
  xx[214] = xx[338] + xx[57] + xx[3] * xx[381];
  xx[365] = xx[201];
  xx[366] = xx[212];
  xx[367] = xx[214];
  xx[222] = xx[217] * state[25] - xx[36] * state[24];
  xx[36] = xx[37] * state[25] - xx[232] * state[24];
  xx[37] = xx[213] * state[25] - xx[204] * state[24];
  xx[368] = xx[13] * state[23] - xx[222] * state[25];
  xx[369] = xx[16] * state[23] - xx[36] * state[25];
  xx[370] = xx[63] * state[23] - xx[37] * state[25];
  xx[13] = state[18] * state[23];
  xx[16] = state[17] * state[23];
  xx[63] = xx[197] + xx[16];
  xx[197] = state[18] * state[25];
  xx[382] = - xx[13];
  xx[383] = xx[63];
  xx[384] = - xx[197];
  pm_math_Vector3_cross_ra(xx + 352, xx + 382, xx + 385);
  xx[382] = (xx[13] * state[16] + xx[385]) * xx[3] - state[25];
  xx[383] = xx[3] * (xx[386] - xx[63] * state[16]);
  xx[384] = state[23] + (xx[197] * state[16] + xx[387]) * xx[3];
  xx[13] = xx[343] + xx[12];
  xx[63] = xx[199] * xx[196];
  xx[197] = xx[13] * xx[196];
  xx[204] = xx[13] * xx[195];
  xx[213] = xx[206] + xx[204];
  xx[385] = xx[63];
  xx[386] = xx[197];
  xx[387] = - xx[213];
  pm_math_Vector3_cross_ra(xx + 194, xx + 385, xx + 388);
  xx[385] = xx[13] + (xx[193] * xx[63] + xx[388]) * xx[3];
  xx[386] = (xx[193] * xx[197] + xx[389]) * xx[3] - xx[199];
  xx[387] = xx[3] * (xx[390] - xx[213] * xx[193]);
  xx[388] = xx[330] + xx[49] + xx[3] * xx[373];
  xx[389] = xx[333] + xx[52] + xx[3] * xx[376];
  xx[390] = xx[336] + xx[55] + xx[3] * xx[379];
  xx[49] = xx[222] * state[24] - xx[2] * state[23];
  xx[2] = xx[36] * state[24] - xx[15] * state[23];
  xx[15] = xx[37] * state[24] - xx[34] * state[23];
  xx[50] = xx[49];
  xx[51] = xx[2];
  xx[52] = xx[15];
  xx[34] = state[19] * state[23];
  xx[36] = state[19] * state[24];
  xx[37] = xx[16] + xx[133];
  xx[53] = - xx[34];
  xx[54] = - xx[36];
  xx[55] = xx[37];
  pm_math_Vector3_cross_ra(xx + 352, xx + 53, xx + 330);
  xx[53] = state[24] + (xx[34] * state[16] + xx[330]) * xx[3];
  xx[54] = (xx[36] * state[16] + xx[331]) * xx[3] - state[23];
  xx[55] = xx[3] * (xx[332] - xx[37] * state[16]);
  xx[16] = xx[204] + xx[1];
  xx[1] = xx[13] * xx[194];
  xx[34] = xx[203] * xx[194];
  xx[330] = - xx[16];
  xx[331] = xx[1];
  xx[332] = xx[34];
  pm_math_Vector3_cross_ra(xx + 194, xx + 330, xx + 333);
  xx[330] = xx[3] * (xx[333] - xx[16] * xx[193]);
  xx[331] = xx[203] + (xx[193] * xx[1] + xx[334]) * xx[3];
  xx[332] = (xx[193] * xx[34] + xx[335]) * xx[3] - xx[13];
  pm_math_Matrix3x3_xform_ra(xx + 318, xx + 111, xx + 333);
  xx[318] = - (xx[103] * xx[393]);
  xx[319] = - (xx[103] * xx[396]);
  xx[320] = - (xx[103] * xx[399]);
  pm_math_Quaternion_xform_ra(xx + 129, xx + 318, xx + 321);
  xx[318] = xx[92];
  xx[319] = xx[102];
  xx[320] = xx[106];
  xx[1] = xx[103] * xx[14];
  xx[13] = xx[1] * xx[33];
  xx[14] = xx[12] * xx[103];
  xx[12] = xx[14] * xx[33];
  xx[16] = xx[1] * xx[23] + xx[14] * xx[32];
  xx[324] = xx[13];
  xx[325] = xx[12];
  xx[326] = - xx[16];
  pm_math_Vector3_cross_ra(xx + 318, xx + 324, xx + 336);
  xx[324] = (xx[336] - xx[26] * xx[13]) * xx[3] - xx[14];
  xx[325] = (xx[337] - xx[26] * xx[12]) * xx[3] + xx[1];
  xx[326] = xx[3] * (xx[338] + xx[16] * xx[26]);
  pm_math_Vector3_cross_ra(xx + 4, xx + 324, xx + 12);
  pm_math_Quaternion_xform_ra(xx + 129, xx + 12, xx + 324);
  xx[373] = pm_math_Vector3_dot_ra(xx + 339, xx + 207) + pm_math_Vector3_dot_ra
    (xx + 346, xx + 293) + pm_math_Vector3_dot_ra(xx + 358, xx + 361) * xx[3];
  xx[374] = pm_math_Vector3_dot_ra(xx + 365, xx + 301) + pm_math_Vector3_dot_ra
    (xx + 368, xx + 309) + pm_math_Vector3_dot_ra(xx + 382, xx + 385) * xx[3];
  xx[375] = pm_math_Vector3_dot_ra(xx + 388, xx + 315) + pm_math_Vector3_dot_ra
    (xx + 50, xx + 327) + pm_math_Vector3_dot_ra(xx + 53, xx + 330) * xx[3];
  xx[376] = xx[198] * xx[49] - (xx[333] + xx[223] + xx[321] + xx[3] * xx[324] -
    xx[201] * xx[234]);
  xx[377] = xx[198] * xx[2] - (xx[334] + xx[307] + xx[322] + xx[3] * xx[325] -
    xx[212] * xx[234]);
  xx[378] = xx[198] * xx[15] - (xx[335] + xx[314] + xx[323] + xx[3] * xx[326] -
    xx[214] * xx[234]);
  zeroMajor(1, 6, ii + 0, xx + 373);
  pm_math_Vector3_cross_ra(xx + 4, xx + 111, xx + 12);
  pm_math_Vector3_cross_ra(xx + 4, xx + 12, xx + 49);
  pm_math_Quaternion_inverseXform_ra(xx + 189, xx + 49, xx + 52);
  xx[1] = xx[3] * xx[22] * xx[31];
  xx[2] = xx[1] * inputDot[0];
  xx[15] = xx[344] + xx[2];
  xx[16] = xx[344] + xx[15];
  xx[22] = xx[3] * xx[31] * xx[31];
  xx[31] = (xx[0] - xx[22]) * inputDot[0];
  xx[34] = xx[103] * xx[31];
  xx[307] = xx[0];
  xx[308] = xx[39];
  xx[309] = xx[39];
  xx[310] = xx[39];
  xx[311] = xx[39];
  xx[312] = xx[39];
  xx[313] = xx[225];
  pm_math_Quaternion_xform_ra(xx + 129, xx + 111, xx + 49);
  xx[36] = state[0] + xx[17];
  xx[37] = 6.938893903907228e-18;
  xx[55] = xx[37] * xx[195];
  xx[56] = xx[37] * xx[194];
  xx[57] = xx[3] * (xx[66] - xx[65]);
  xx[63] = state[1] - xx[57];
  xx[65] = state[2] - xx[95] + xx[0];
  xx[321] = - xx[195];
  xx[322] = - xx[196];
  xx[323] = xx[193];
  xx[324] = xx[194];
  xx[325] = xx[49] + xx[36] - (xx[193] * xx[55] + xx[196] * xx[56]) * xx[3];
  xx[326] = xx[3] * (xx[193] * xx[56] - xx[196] * xx[55]) + xx[50] + xx[63];
  xx[327] = (xx[194] * xx[56] + xx[195] * xx[55]) * xx[3] - xx[37] + xx[51] +
    xx[65];
  bb[1] = sm_core_compiler_computeProximityInfoBrickCylinder(
    simulation_b048d748_1_geometry_3(NULL), simulation_b048d748_1_geometry_2
    (NULL), (pm_math_Transform3 *)(xx + 307), (pm_math_Transform3 *)(xx + 321),
    xx + 49, (pm_math_Vector3 *)(xx + 197), (pm_math_Vector3 *)(xx + 206),
    (pm_math_Vector3 *)(xx + 212), (pm_math_Vector3 *)(xx + 222));
  xx[328] = xx[39];
  xx[329] = xx[39];
  xx[330] = xx[0];
  xx[331] = xx[39];
  xx[332] = xx[39];
  xx[333] = xx[39];
  xx[334] = - xx[37];
  xx[37] = xx[342] + xx[31];
  xx[50] = xx[343] + inputDot[1];
  xx[292] = state[7];
  xx[293] = state[8];
  xx[294] = state[9];
  pm_math_Quaternion_inverseXform_ra(xx + 129, xx + 292, xx + 335);
  xx[51] = xx[335] + state[11];
  xx[55] = xx[336] - state[10];
  xx[292] = xx[12] + xx[51];
  xx[293] = xx[13] + xx[55];
  xx[294] = xx[14] + xx[337];
  pm_math_Quaternion_inverseXform_ra(xx + 189, xx + 292, xx + 12);
  xx[56] = xx[103] * inputDot[1];
  xx[358] = xx[37];
  xx[359] = xx[50];
  xx[360] = xx[15];
  xx[361] = xx[12] - xx[56];
  xx[362] = xx[13] + xx[34];
  xx[363] = xx[14];
  xx[12] = 1000.0;
  xx[13] = 1.0e-4;
  xx[14] = 0.3;
  xx[66] = 0.2119573811760597;
  xx[92] = 9.126024771145405e-4;
  sm_core_compiler_computeSpatialContactWrenches(
    0, 1, bb[1], xx + 49, (const pm_math_Vector3 *)(xx + 197), (const
    pm_math_Vector3 *)(xx + 206), (const pm_math_Vector3 *)(xx + 212), (const
    pm_math_Vector3 *)(xx + 222),
    (const pm_math_Transform3 *)(xx + 307), (const pm_math_Transform3 *)(xx +
    328), (const pm_math_Transform3 *)(xx + 307), (const pm_math_Transform3 *)
    (xx + 321), NULL, (const pm_math_SpatialVector *)(xx + 358),
    0, 1, 1.0e6, xx[12], xx[13], xx[14], xx[66], xx[92], NULL, NULL,
    NULL, (pm_math_SpatialVector *)(xx + 365));
  xx[49] = inputDot[0] * inputDot[1];
  xx[102] = xx[1] * xx[49];
  xx[1] = - xx[25];
  xx[25] = sin(xx[1]);
  xx[106] = xx[0] - xx[3] * xx[25] * xx[25];
  xx[197] = (xx[52] - xx[16] * xx[34] - xx[103] * inputDdot[1]) * xx[27] - xx
    [368];
  xx[198] = xx[27] * (xx[53] - xx[16] * xx[56] - xx[103] * xx[102] + xx[103] *
                      xx[106] * inputDdot[0]) - xx[369];
  xx[199] = xx[27] * (xx[54] + (xx[342] + xx[37]) * xx[34] + (xx[343] + xx[50]) *
                      xx[56]) - (xx[370] + input[4]);
  pm_math_Quaternion_xform_ra(xx + 189, xx + 197, xx + 52);
  xx[321] = xx[46];
  xx[322] = xx[68];
  xx[323] = xx[71];
  xx[324] = xx[73];
  xx[325] = xx[36] - xx[17];
  xx[326] = xx[57] + xx[63];
  xx[327] = xx[95] - xx[0] + xx[65];
  bb[1] = sm_core_compiler_computeProximityInfoCxpolyBrick(
    simulation_b048d748_1_geometry_0(NULL), simulation_b048d748_1_geometry_3
    (NULL), (pm_math_Transform3 *)(xx + 321), (pm_math_Transform3 *)(xx + 307),
    xx + 16, (pm_math_Vector3 *)(xx + 197), (pm_math_Vector3 *)(xx + 206),
    (pm_math_Vector3 *)(xx + 212), (pm_math_Vector3 *)(xx + 222));
  xx[328] = state[10];
  xx[329] = state[11];
  xx[330] = state[12];
  xx[331] = xx[51];
  xx[332] = xx[55];
  xx[333] = xx[337];
  sm_core_compiler_computeSpatialContactWrenches(
    0, 1, bb[1], xx + 16, (const pm_math_Vector3 *)(xx + 206), (const
    pm_math_Vector3 *)(xx + 197), (const pm_math_Vector3 *)(xx + 222), (const
    pm_math_Vector3 *)(xx + 212),
    (const pm_math_Transform3 *)(xx + 307), (const pm_math_Transform3 *)(xx +
    307), (const pm_math_Transform3 *)(xx + 307), (const pm_math_Transform3 *)
    (xx + 321), NULL, (const pm_math_SpatialVector *)(xx + 328),
    0, 1, 1300.0, xx[12], xx[13], 0.4, 0.3166621126427172, 9.173266606314467e-4,
    NULL, NULL,
    NULL, (pm_math_SpatialVector *)(xx + 358));
  xx[197] = xx[51];
  xx[198] = xx[55];
  xx[199] = xx[337];
  pm_math_Vector3_cross_ra(xx + 4, xx + 197, xx + 55);
  xx[197] = - xx[335];
  xx[198] = - xx[336];
  xx[199] = - xx[337];
  pm_math_Vector3_cross_ra(xx + 4, xx + 197, xx + 206);
  xx[197] = xx[55] + xx[206];
  xx[198] = xx[56] + xx[207];
  xx[199] = xx[57] + xx[208];
  pm_math_Matrix3x3_xform_ra(xx + 76, xx + 197, xx + 55);
  xx[16] = xx[52] - xx[361] + xx[55];
  xx[17] = xx[53] - xx[362] + xx[56];
  xx[76] = xx[16];
  xx[77] = xx[17];
  xx[78] = xx[54] - xx[363] + xx[57];
  xx[34] = pm_math_Vector3_dot_ra(xx + 18, xx + 76);
  xx[36] = pm_math_Vector3_dot_ra(xx + 86, xx + 76);
  xx[46] = pm_math_Vector3_dot_ra(xx + 96, xx + 76);
  pm_math_Vector3_cross_ra(xx + 4, xx + 4, xx + 55);
  xx[4] = xx[37];
  xx[5] = xx[50];
  xx[6] = xx[15];
  xx[76] = xx[37] * xx[74];
  xx[77] = xx[50] * xx[74];
  xx[78] = xx[15] * xx[75];
  pm_math_Vector3_cross_ra(xx + 4, xx + 76, xx + 79);
  xx[4] = xx[31];
  xx[5] = inputDot[1];
  xx[6] = xx[2];
  pm_math_Vector3_cross_ra(xx + 342, xx + 4, xx + 76);
  xx[4] = xx[79] - xx[365] + (xx[76] - xx[102] + xx[106] * inputDdot[0]) * xx[74];
  xx[5] = xx[80] - xx[366] + (xx[77] + inputDdot[1]) * xx[74];
  xx[6] = xx[81] - xx[367] + xx[75] * (xx[78] - (xx[22] - xx[0]) * xx[49] - xx[3]
    * cos(xx[1]) * xx[25] * inputDdot[0]);
  pm_math_Quaternion_xform_ra(xx + 189, xx + 4, xx + 49);
  pm_math_Vector3_cross_ra(xx + 111, xx + 52, xx + 4);
  xx[76] = - xx[114];
  xx[77] = - xx[117];
  xx[78] = - xx[120];
  xx[79] = - xx[115];
  xx[80] = - xx[118];
  xx[81] = - xx[121];
  xx[82] = xx[126];
  xx[83] = xx[127];
  xx[84] = xx[128];
  pm_math_Matrix3x3_xform_ra(xx + 76, xx + 197, xx + 52);
  xx[1] = xx[55] - xx[358] + xx[49] + xx[4] + xx[52];
  xx[2] = xx[56] - xx[359] + xx[50] + xx[5] + xx[53] + xx[16];
  xx[4] = xx[57] - xx[360] + xx[51] + xx[6] + xx[54];
  xx[49] = - xx[34];
  xx[50] = - xx[36];
  xx[51] = - xx[46];
  xx[52] = xx[17] - xx[1];
  xx[53] = - xx[2];
  xx[54] = - xx[4];
  solveSymmetricPosDef(xx + 153, xx + 49, 6, 1, xx + 321, xx + 327);
  pm_math_Matrix3x3_xform_ra(xx + 76, xx + 18, xx + 49);
  pm_math_Matrix3x3_xform_ra(xx + 76, xx + 86, xx + 18);
  pm_math_Matrix3x3_xform_ra(xx + 76, xx + 96, xx + 52);
  xx[443] = xx[49];
  xx[444] = xx[18];
  xx[445] = xx[52];
  xx[446] = xx[30];
  xx[447] = xx[38];
  xx[448] = xx[41];
  xx[449] = xx[50];
  xx[450] = xx[19];
  xx[451] = xx[53];
  xx[452] = xx[147] - xx[138] + xx[118];
  xx[453] = xx[48];
  xx[454] = xx[122];
  xx[455] = xx[51];
  xx[456] = xx[20];
  xx[457] = xx[54];
  xx[458] = xx[150] - xx[141] + xx[119];
  xx[459] = xx[151] - xx[142] - xx[116];
  xx[460] = xx[125];
  xx[461] = xx[60];
  xx[462] = xx[89];
  xx[463] = xx[99];
  xx[464] = xx[67];
  xx[465] = xx[72];
  xx[466] = xx[126];
  xx[467] = xx[61];
  xx[468] = xx[90];
  xx[469] = xx[100];
  xx[470] = xx[59];
  xx[471] = xx[58];
  xx[472] = xx[127];
  xx[473] = xx[62];
  xx[474] = xx[91];
  xx[475] = xx[101];
  xx[476] = xx[69];
  xx[477] = xx[70];
  xx[478] = xx[128];
  solveSymmetricPosDef(xx + 153, xx + 443, 6, 6, xx + 479, xx + 48);
  xx[18] = xx[500];
  xx[19] = xx[506];
  xx[20] = xx[512];
  xx[5] = 9.806649999999999;
  xx[6] = xx[5] * state[4];
  xx[15] = xx[5] * state[5];
  xx[48] = xx[3] * (xx[6] * state[6] - xx[15] * state[3]);
  xx[49] = (xx[6] * state[3] + xx[15] * state[6]) * xx[3];
  xx[50] = xx[5] - (xx[6] * state[4] + xx[15] * state[5]) * xx[3];
  xx[6] = pm_math_Vector3_dot_ra(xx + 18, xx + 48);
  xx[15] = xx[324] - xx[6];
  xx[18] = xx[501];
  xx[19] = xx[507];
  xx[20] = xx[513];
  xx[16] = pm_math_Vector3_dot_ra(xx + 18, xx + 48);
  xx[18] = xx[325] - xx[16];
  xx[51] = xx[502];
  xx[52] = xx[508];
  xx[53] = xx[514];
  xx[19] = pm_math_Vector3_dot_ra(xx + 51, xx + 48);
  xx[20] = xx[326] - xx[19];
  xx[51] = state[23];
  xx[52] = state[24];
  xx[53] = state[25];
  xx[22] = - state[16];
  xx[54] = xx[22];
  xx[55] = xx[85];
  xx[56] = xx[93];
  xx[57] = xx[110];
  xx[58] = state[20];
  xx[59] = state[21];
  xx[60] = state[22];
  pm_math_Quaternion_inverseXform_ra(xx + 54, xx + 58, xx + 61);
  pm_math_Vector3_cross_ra(xx + 51, xx + 61, xx + 54);
  xx[57] = - xx[61];
  xx[58] = - xx[62];
  xx[59] = - xx[63];
  pm_math_Vector3_cross_ra(xx + 51, xx + 57, xx + 67);
  xx[25] = 3.469446951953614e-18;
  xx[30] = xx[25] * state[18];
  xx[31] = xx[25] * state[17];
  xx[76] = state[18];
  xx[77] = state[19];
  xx[78] = xx[22];
  xx[79] = xx[85];
  xx[80] = state[13] - (xx[30] * state[16] + xx[31] * state[19]) * xx[3];
  xx[81] = xx[3] * (xx[31] * state[16] - xx[30] * state[19]) + state[14];
  xx[82] = (xx[31] * state[17] + xx[30] * state[18]) * xx[3] - xx[25] + state[15];
  bb[1] = sm_core_compiler_computeProximityInfoBrickCylinder(
    simulation_b048d748_1_geometry_3(NULL), simulation_b048d748_1_geometry_1
    (NULL), (pm_math_Transform3 *)(xx + 307), (pm_math_Transform3 *)(xx + 76),
    xx + 22, (pm_math_Vector3 *)(xx + 57), (pm_math_Vector3 *)(xx + 70),
    (pm_math_Vector3 *)(xx + 83), (pm_math_Vector3 *)(xx + 86));
  xx[95] = xx[39];
  xx[96] = xx[39];
  xx[97] = xx[0];
  xx[98] = xx[39];
  xx[99] = xx[39];
  xx[100] = xx[39];
  xx[101] = - xx[25];
  xx[110] = state[23];
  xx[111] = state[24];
  xx[112] = state[25];
  xx[113] = xx[61];
  xx[114] = xx[62];
  xx[115] = xx[63];
  sm_core_compiler_computeSpatialContactWrenches(
    0, 1, bb[1], xx + 22, (const pm_math_Vector3 *)(xx + 57), (const
    pm_math_Vector3 *)(xx + 70), (const pm_math_Vector3 *)(xx + 83), (const
    pm_math_Vector3 *)(xx + 86),
    (const pm_math_Transform3 *)(xx + 307), (const pm_math_Transform3 *)(xx + 95),
    (const pm_math_Transform3 *)(xx + 307), (const pm_math_Transform3 *)(xx + 76),
    NULL, (const pm_math_SpatialVector *)(xx + 110),
    0, 1, xx[12], 100.0, xx[13], xx[14], xx[66], xx[92], NULL, NULL,
    NULL, (pm_math_SpatialVector *)(xx + 116));
  xx[12] = (xx[54] + xx[67]) * xx[27] - xx[119];
  xx[13] = (xx[55] + xx[68]) * xx[27] - xx[120];
  xx[14] = (xx[56] + xx[69]) * xx[27] - (input[3] + xx[121]);
  xx[22] = pm_math_Vector3_dot_ra(xx + 216, xx + 12);
  xx[25] = pm_math_Vector3_dot_ra(xx + 247, xx + 12);
  xx[27] = pm_math_Vector3_dot_ra(xx + 251, xx + 12);
  xx[12] = xx[254] * state[23];
  xx[13] = xx[254] * state[24];
  xx[14] = xx[75] * state[25];
  pm_math_Vector3_cross_ra(xx + 51, xx + 12, xx + 54);
  xx[57] = - xx[22];
  xx[58] = - xx[25];
  xx[59] = - xx[27];
  xx[60] = xx[116] - xx[54];
  xx[61] = xx[117] - xx[55];
  xx[62] = xx[118] - xx[56];
  solveSymmetricPosDef(xx + 255, xx + 57, 6, 1, xx + 65, xx + 76);
  xx[443] = xx[39];
  xx[444] = xx[39];
  xx[445] = xx[39];
  xx[446] = xx[254];
  xx[447] = xx[39];
  xx[448] = xx[39];
  xx[449] = xx[39];
  xx[450] = xx[39];
  xx[451] = xx[39];
  xx[452] = xx[39];
  xx[453] = xx[254];
  xx[454] = xx[39];
  xx[455] = xx[39];
  xx[456] = xx[39];
  xx[457] = xx[39];
  xx[458] = xx[39];
  xx[459] = xx[39];
  xx[460] = xx[75];
  xx[461] = xx[219];
  xx[462] = xx[226];
  xx[463] = xx[238];
  xx[464] = xx[39];
  xx[465] = xx[39];
  xx[466] = xx[39];
  xx[467] = xx[220];
  xx[468] = xx[229];
  xx[469] = xx[240];
  xx[470] = xx[39];
  xx[471] = xx[39];
  xx[472] = xx[39];
  xx[473] = xx[221];
  xx[474] = xx[233];
  xx[475] = xx[242];
  xx[476] = xx[39];
  xx[477] = xx[39];
  xx[478] = xx[39];
  solveSymmetricPosDef(xx + 255, xx + 443, 6, 6, xx + 515, xx + 57);
  xx[12] = xx[537];
  xx[13] = xx[543];
  xx[14] = xx[549];
  xx[30] = xx[5] * state[17];
  xx[31] = xx[5] * state[18];
  xx[57] = xx[3] * (xx[30] * state[19] - xx[31] * state[16]);
  xx[58] = (xx[30] * state[16] + xx[31] * state[19]) * xx[3];
  xx[59] = xx[5] - (xx[30] * state[17] + xx[31] * state[18]) * xx[3];
  xx[5] = pm_math_Vector3_dot_ra(xx + 12, xx + 57);
  xx[12] = xx[69] - xx[5];
  xx[60] = xx[538];
  xx[61] = xx[544];
  xx[62] = xx[550];
  xx[13] = pm_math_Vector3_dot_ra(xx + 60, xx + 57);
  xx[14] = xx[70] - xx[13];
  xx[60] = xx[0] - (xx[245] + xx[300]) * xx[3];
  xx[61] = (xx[241] + xx[237]) * xx[3];
  xx[62] = xx[202];
  xx[71] = xx[536];
  xx[72] = xx[542];
  xx[73] = xx[548];
  xx[30] = pm_math_Vector3_dot_ra(xx + 71, xx + 57);
  xx[31] = xx[68] - xx[30];
  xx[71] = - ((xx[134] + xx[43]) * xx[3]);
  xx[72] = xx[3] * (xx[291] - xx[250]);
  xx[73] = (xx[44] + xx[245]) * xx[3] - xx[0];
  xx[74] = xx[497];
  xx[75] = xx[503];
  xx[76] = xx[509];
  xx[0] = pm_math_Vector3_dot_ra(xx + 74, xx + 48);
  xx[37] = xx[103] * xx[123];
  xx[38] = xx[37] * xx[33];
  xx[41] = xx[103] * xx[35];
  xx[35] = xx[41] * xx[33];
  xx[43] = xx[37] * xx[23] + xx[41] * xx[32];
  xx[74] = xx[38];
  xx[75] = xx[35];
  xx[76] = - xx[43];
  pm_math_Vector3_cross_ra(xx + 318, xx + 74, xx + 77);
  xx[74] = (xx[77] - xx[26] * xx[38]) * xx[3] - xx[41];
  xx[75] = (xx[78] - xx[26] * xx[35]) * xx[3] + xx[37];
  xx[76] = xx[3] * (xx[79] + xx[43] * xx[26]);
  pm_math_Quaternion_xform_ra(xx + 129, xx + 74, xx + 77);
  xx[32] = xx[103] * xx[33];
  xx[74] = (xx[109] + xx[32] * xx[33]) * xx[3] - xx[103];
  xx[75] = - ((xx[32] * xx[26] + xx[104] * xx[23]) * xx[3]);
  xx[76] = xx[3] * (xx[105] - xx[32] * xx[23]);
  pm_math_Quaternion_xform_ra(xx + 129, xx + 74, xx + 80);
  xx[23] = xx[234] * xx[195];
  xx[26] = xx[234] * xx[196];
  xx[74] = xx[533];
  xx[75] = xx[539];
  xx[76] = xx[545];
  xx[32] = pm_math_Vector3_dot_ra(xx + 74, xx + 57);
  xx[74] = xx[498];
  xx[75] = xx[504];
  xx[76] = xx[510];
  xx[33] = pm_math_Vector3_dot_ra(xx + 74, xx + 48);
  xx[74] = xx[534];
  xx[75] = xx[540];
  xx[76] = xx[546];
  xx[35] = pm_math_Vector3_dot_ra(xx + 74, xx + 57);
  xx[74] = xx[499];
  xx[75] = xx[505];
  xx[76] = xx[511];
  xx[37] = pm_math_Vector3_dot_ra(xx + 74, xx + 48);
  xx[48] = xx[535];
  xx[49] = xx[541];
  xx[50] = xx[547];
  xx[38] = pm_math_Vector3_dot_ra(xx + 48, xx + 57);
  xx[83] = - (xx[373] + xx[15] * xx[210] + xx[18] * xx[24] + xx[20] * xx[200] +
              xx[210] * inputDdot[0] + xx[12] * xx[296] + xx[14] * xx[228]);
  xx[84] = - (xx[374] + xx[15] * xx[230] + xx[18] * xx[29] + xx[20] * xx[42] +
              xx[230] * inputDdot[0] + (bb[0] ? xx[39] : pm_math_Vector3_dot_ra
    (xx + 60, xx + 301)) * inputDdot[1] + xx[31] * xx[305] + xx[14] * xx[215]);
  xx[85] = - (xx[375] + xx[15] * xx[124] + xx[18] * xx[28] + xx[20] * xx[40] +
              xx[124] * inputDdot[0] + (bb[0] ? xx[39] : pm_math_Vector3_dot_ra
    (xx + 71, xx + 315)) * inputDdot[1] + xx[31] * xx[205] + xx[12] * xx[211]);
  xx[86] = - (xx[376] + (xx[321] - xx[0]) * xx[227] + xx[15] * xx[345] + xx[18] *
              xx[357] + xx[20] * xx[239] + (bb[0] ? xx[39] : - (xx[306] + xx[77]))
              * inputDdot[0] + (bb[0] ? xx[39] : - (xx[80] + (xx[195] * xx[23] +
    xx[196] * xx[26]) * xx[3] - xx[234])) * inputDdot[1] + (xx[65] - xx[32]) *
              xx[45] + xx[31] * xx[351] + xx[12] * xx[364]);
  xx[87] = - (xx[377] + (xx[322] - xx[33]) * xx[227] + xx[15] * xx[298] + xx[18]
              * xx[64] + xx[20] * xx[21] + (bb[0] ? xx[39] : - (xx[372] + xx[236]
    + xx[78])) * inputDdot[0] + (bb[0] ? xx[39] : - (xx[81] - (xx[193] * xx[26]
    + xx[194] * xx[23]) * xx[3])) * inputDdot[1] + (xx[66] - xx[35]) * xx[45] +
              xx[31] * xx[11] + xx[12] * xx[47]);
  xx[88] = - (xx[378] + (xx[323] - xx[37]) * xx[227] + xx[15] * xx[299] + xx[18]
              * xx[94] + xx[20] * xx[107] + (bb[0] ? xx[39] : - (xx[231] + xx[79]))
              * inputDdot[0] + (bb[0] ? xx[39] : - (xx[3] * (xx[193] * xx[23] -
    xx[194] * xx[26]) + xx[82])) * inputDdot[1] + (xx[67] - xx[38]) * xx[45] +
              xx[31] * xx[108] + xx[12] * xx[244]);
  memcpy(xx + 306, xx + 407, 36 * sizeof(double));
  factorAndSolveSymmetric(xx + 306, 6, xx + 65, ii + 0, xx + 83, xx + 57, xx +
    365);
  xx[65] = xx[60] * xx[227] - xx[34];
  xx[66] = xx[61] * xx[227] - xx[36];
  xx[67] = xx[62] * xx[227] - xx[46];
  xx[68] = xx[57] * xx[210] + xx[58] * xx[230] + xx[59] * xx[124] + xx[60] * xx
    [345] + xx[61] * xx[298] + xx[62] * xx[299] - (xx[1] - xx[17]);
  xx[69] = xx[57] * xx[24] + xx[58] * xx[29] + xx[59] * xx[28] + xx[60] * xx[357]
    + xx[61] * xx[64] + xx[62] * xx[94] - xx[2];
  xx[70] = xx[57] * xx[200] + xx[58] * xx[42] + xx[59] * xx[40] + xx[60] * xx
    [239] + xx[61] * xx[21] + xx[62] * xx[107] - xx[4];
  solveSymmetricPosDef(xx + 153, xx + 65, 6, 1, xx + 71, xx + 77);
  xx[1] = state[16];
  xx[2] = state[17];
  xx[3] = state[18];
  xx[4] = state[19];
  pm_math_Quaternion_compDeriv_ra(xx + 1, xx + 51, xx + 40);
  xx[48] = xx[60] * xx[45] - xx[22];
  xx[49] = xx[61] * xx[45] - xx[25];
  xx[50] = xx[62] * xx[45] - xx[27];
  xx[51] = xx[58] * xx[305] + xx[59] * xx[205] + xx[60] * xx[351] + xx[61] * xx
    [11] + xx[62] * xx[108] - (xx[54] - xx[116]);
  xx[52] = xx[57] * xx[296] + xx[59] * xx[211] + xx[60] * xx[364] + xx[61] * xx
    [47] + xx[62] * xx[244] - (xx[55] - xx[117]);
  xx[53] = xx[57] * xx[228] + xx[58] * xx[215] - (xx[56] - xx[118]);
  solveSymmetricPosDef(xx + 255, xx + 48, 6, 1, xx + 20, xx + 54);
  deriv[0] = state[7];
  deriv[1] = state[8];
  deriv[2] = state[9];
  deriv[3] = xx[7];
  deriv[4] = xx[8];
  deriv[5] = xx[9];
  deriv[6] = xx[10];
  deriv[7] = xx[71] - xx[0];
  deriv[8] = xx[72] - xx[33];
  deriv[9] = xx[73] - xx[37];
  deriv[10] = xx[74] - xx[6];
  deriv[11] = xx[75] - xx[16];
  deriv[12] = xx[76] - xx[19];
  deriv[13] = state[20];
  deriv[14] = state[21];
  deriv[15] = state[22];
  deriv[16] = xx[40];
  deriv[17] = xx[41];
  deriv[18] = xx[42];
  deriv[19] = xx[43];
  deriv[20] = xx[20] - xx[32];
  deriv[21] = xx[21] - xx[35];
  deriv[22] = xx[22] - xx[38];
  deriv[23] = xx[23] - xx[30];
  deriv[24] = xx[24] - xx[5];
  deriv[25] = xx[25] - xx[13];
  errorResult[0] = xx[39];
  return NULL;
}

PmfMessageId simulation_b048d748_1_numJacPerturbLoBounds(const
  RuntimeDerivedValuesBundle *rtdv, const int *eqnEnableFlags, const double
  *state, const int *modeVector, const double *input, const double *inputDot,
  const double *inputDdot, const double *discreteState, double *bounds, double
  *errorResult, NeuDiagnosticManager *neDiagMgr)
{
  const double *rtdvd = rtdv->mDoubles.mValues;
  const int *rtdvi = rtdv->mInts.mValues;
  double xx[2];
  (void) rtdvd;
  (void) rtdvi;
  (void) eqnEnableFlags;
  (void) state;
  (void) modeVector;
  (void) input;
  (void) inputDot;
  (void) inputDdot;
  (void) discreteState;
  (void) neDiagMgr;
  xx[0] = 1.0e-9;
  xx[1] = 1.0e-8;
  bounds[0] = xx[0];
  bounds[1] = xx[0];
  bounds[2] = xx[0];
  bounds[3] = xx[1];
  bounds[4] = xx[1];
  bounds[5] = xx[1];
  bounds[6] = xx[1];
  bounds[7] = xx[0];
  bounds[8] = xx[0];
  bounds[9] = xx[0];
  bounds[10] = xx[1];
  bounds[11] = xx[1];
  bounds[12] = xx[1];
  bounds[13] = xx[0];
  bounds[14] = xx[0];
  bounds[15] = xx[0];
  bounds[16] = xx[1];
  bounds[17] = xx[1];
  bounds[18] = xx[1];
  bounds[19] = xx[1];
  bounds[20] = xx[0];
  bounds[21] = xx[0];
  bounds[22] = xx[0];
  bounds[23] = xx[1];
  bounds[24] = xx[1];
  bounds[25] = xx[1];
  errorResult[0] = 0.0;
  return NULL;
}

PmfMessageId simulation_b048d748_1_numJacPerturbHiBounds(const
  RuntimeDerivedValuesBundle *rtdv, const int *eqnEnableFlags, const double
  *state, const int *modeVector, const double *input, const double *inputDot,
  const double *inputDdot, const double *discreteState, double *bounds, double
  *errorResult, NeuDiagnosticManager *neDiagMgr)
{
  const double *rtdvd = rtdv->mDoubles.mValues;
  const int *rtdvi = rtdv->mInts.mValues;
  double xx[2];
  (void) rtdvd;
  (void) rtdvi;
  (void) eqnEnableFlags;
  (void) state;
  (void) modeVector;
  (void) input;
  (void) inputDot;
  (void) inputDdot;
  (void) discreteState;
  (void) neDiagMgr;
  xx[0] = +pmf_get_inf();
  xx[1] = 0.1;
  bounds[0] = xx[0];
  bounds[1] = xx[0];
  bounds[2] = xx[0];
  bounds[3] = xx[1];
  bounds[4] = xx[1];
  bounds[5] = xx[1];
  bounds[6] = xx[1];
  bounds[7] = xx[0];
  bounds[8] = xx[0];
  bounds[9] = xx[0];
  bounds[10] = xx[0];
  bounds[11] = xx[0];
  bounds[12] = xx[0];
  bounds[13] = xx[0];
  bounds[14] = xx[0];
  bounds[15] = xx[0];
  bounds[16] = xx[1];
  bounds[17] = xx[1];
  bounds[18] = xx[1];
  bounds[19] = xx[1];
  bounds[20] = xx[0];
  bounds[21] = xx[0];
  bounds[22] = xx[0];
  bounds[23] = xx[0];
  bounds[24] = xx[0];
  bounds[25] = xx[0];
  errorResult[0] = 0.0;
  return NULL;
}
