...
std::vector<std::string> GPS::break_message_down(const std::string &message)
{
    ...
    int curr_pos = NMEA::MESSAGE_TYPE_STARTING_STRING_INDEX;
...
std::string GPS::determine_NMEA_type(const std::vector<std::string> &nmea_message_parts)
{
    ...
    return nmea_message_parts[NMEA::MESSAGE_TYPE_IDX];
}

void GPS::parse_NMEA_type(const std::string nmea_message_type, const std::vector<std::string> &nmea_message_parts)
{
    if (nmea_message_type == NMEA::RMC::RMC)
    {
        std::cout << "Got RMC output type " << "\n";
        std::string status = nmea_message_parts[NMEA::RMC::STAUS_IDX];
        char status_character = static_cast<char>(status[0]);
        if (status_character == NMEA::RMC::BAD_STATUS_CHARACTER)
        {
            valid = false;
    ...
    else if (nmea_message_type == NMEA::GGA::GGA)
    {
        std::cout << "Got GGA output type " << "\n";
        std::string status = nmea_message_parts[NMEA::GGA::STAUS_IDX];
        int status_int = stoi(status);
        if (status_int == NMEA::GGA::BAD_STATUS_NUMBER)
        {
            valid = false;
            std::cout << "Data is not valid, failed to update GPS_info" << "\n";
        }
...
{
    float time = stof(nmea_message_parts[NMEA::TIME_IDX]);
    float latitude = convert_latitude(nmea_message_parts[NMEA::RMC::LATITUDE_IDX]);
    float longitude = convert_longitude(nmea_message_parts[NMEA::RMC::LONGITUDE_IDX]);
    float speed = convert_speed_to_meter_per_seconds(nmea_message_parts[NMEA::RMC::SPEED_IDX]);
...
{
    float time = stof(nmea_message_parts[NMEA::TIME_IDX]);
    float altitude = stof(nmea_message_parts[NMEA::GGA::ALTITUDE_INDEX]);
